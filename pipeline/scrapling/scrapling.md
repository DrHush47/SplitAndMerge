...
> **Место в каскаде:** Ур.3 — Scrapling StealthySession. Полная схема эскалации: [`../docs/cascade.md`](../docs/cascade.md)

**Когда применять Scrapling:** anti-bot защита (Cloudflare Turnstile, DataDome); пере-парсинг с `--adaptive`.

**Когда НЕ применять:** простые статические URL (быстрее через Crawl4AI).

> Условия входа/выхода по каскаду — только в [`../docs/cascade.md`](../docs/cascade.md), здесь не дублируются.
...
> venv, выходные файлы, очистка `workspace/` — только в [`../docs/knowledge.md`](../docs/knowledge.md) §0.1, здесь не дублируются. Специфика уровня: таймаут в секундах → `* 1000` для Playwright (ms); браузеры — `%USERPROFILE%\AppData\Local\ms-playwright\`; бесплатно.
