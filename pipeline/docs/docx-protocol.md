...
> Канон кода — только в `../templates/docx_orchestrator.py`, здесь не дублируется. Для задач на 5+ блоков правок копируй его как основу. Ниже — схема-заглушка (не запускать, тела функций — в .py-файле).

```python
#!/usr/bin/env python3
"""Оркестратор правок .docx через python-docx (полный канон — в ../templates/docx_orchestrator.py)."""
from docx import Document

FILE = "результат.docx"
doc = Document(FILE)

# =================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (см. docx_orchestrator.py) ===================

def set_run_font(run, name="Times New Roman", size=12, bold=None, italic=None):
    """См. ../templates/docx_orchestrator.py — здесь не дублируется."""
    ...
...
def insert_heading_before(doc, anchor_text, heading_text, check_duplicate=True):
    """См. ../templates/docx_orchestrator.py — здесь не дублируется (канон §1.6)."""
    ...

def replace_cross_run_text(paragraph, old, new):
    """См. ../templates/docx_orchestrator.py и §5.1 — здесь не дублируется."""
    ...
...
def renumber_references(doc, ref_start_pattern='ЛИТЕРАТУРА', ref_end_pattern='СВЕДЕНИЯ ОБ АВТОРАХ'):
    """См. ../templates/docx_orchestrator.py и §1.8 — здесь не дублируется."""
    ...
...
# Блок B: Заголовки — см. пример в docx_orchestrator.py
# Блок D: Правки в таблице — см. пример в docx_orchestrator.py
# Блок G: Длинные вставки в конец — см. пример в docx_orchestrator.py
doc.save(FILE)
```
...
### 5.6.2 Fallback при отсутствии якоря — см. §1.7 (канон), duplicate-check — см. §1.6.

### 5.6.3 Нумерация двузначных номеров — см. §1.8 (канон: regex `^\d+\.\s`).

### 5.6.4 Потеря форматирования при cross-run замене — см. §5.1 (канон).
...
> v1 (OfficeCLI) заменена v2 (python-docx): быстрее (~0.01 сек), 80% правок с первой попытки. OfficeCLI — только резерв. Детали A/B-теста удалены как исторический мусор.

---

## 13. Эмпирика (кратко, тест 8 правок 2026-07-14: 7/8 с первого прогона)

> Надёжно: вставка заголовков через `OxmlElement` + `addprevious()`, замена в ячейках, поля, `add_paragraph()`, нумерация через regex. Рискованно: inline-подзаголовки (§5.6.1), fallback без duplicate-check (§1.6), cross-run с разным форматированием (§5.1). Silent failures не ловит самоотчёт — обязательна программная валидация Рецензентом.
